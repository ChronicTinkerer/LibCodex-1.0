...
-- v2 slots: 1:id, 2:label (Z85), 3:primaryStat (Z85), 4:classID, 5:role (Z85)

local LibCodex = LibStub("LibCodex-1.0")
local Specs = LibCodex.CollectionFactory.New("Specs", {
    keyField = "id",
    searchFields = { "label", "role" },
})
...
function Specs:_DecodeV2Row(slots, schemaVersion, build)
    ...
    if type(slots[3]) == "string" then entry.primaryStat = decodeZ85String(slots[3]) end
    if type(slots[4]) == "number" then entry.classID     = slots[4] end
    if type(slots[5]) == "string" then entry.role        = decodeZ85String(slots[5]) end
