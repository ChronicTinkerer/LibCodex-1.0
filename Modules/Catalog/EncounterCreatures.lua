...
local EncounterCreatures = LibCodex.CollectionFactory.New("EncounterCreatures", { keyField = "id", searchFields = { "label","description" } })
...
function EncounterCreatures:_DecodeV2Row(slots, schemaVersion, build)
    ...
    if type(slots[3]) == "string" then entry.description = decodeZ85String(slots[3]) end
    if type(slots[4]) == "number" then entry.creatureDisplayInfoID = slots[4] end
    if type(slots[5]) == "number" then entry.fileDataID = slots[5] end
    if type(slots[6]) == "number" then entry.journalEncounterID = slots[6] end
    if type(slots[7]) == "number" then entry.orderIndex = slots[7] end
    if type(slots[8]) == "number" then entry.uiModelSceneID = slots[8] end
