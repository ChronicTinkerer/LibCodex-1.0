...
local Zones = LibCodex.CollectionFactory.New("Zones", { keyField = "id", searchFields = { "label" } })
...
function Zones:_DecodeV2Row(slots, schemaVersion, build)
    ...
    if type(slots[3]) == "string" then entry.type     = decodeZ85String(slots[3]) end
    if type(slots[4]) == "number" then entry.parentID = slots[4] end
